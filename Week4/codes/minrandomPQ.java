...
public class minrandomPQ<Key extends Comparable<Key>>
{
	...
	public static void main(String[] args) {
		minrandomPQ<Integer> q = new minrandomPQ<>();
		try
		{
			q.push(1);
			q.push(2);
	...
	public Key sample()
	{
		int id = StdRandom.uniform(0, i);
		return internal_pq[id];
	}
	public Key random_remove()
	{
		int id = StdRandom.uniform(0, i);
		Key temp = internal_pq[id];
		internal_pq[id] = internal_pq[i-1];
		internal_pq[i-1] = temp;
		i--;
		sink(id);
		swim(id);
		return temp;
